...
import { drawForestHighlights } from "./forestHighlights";
import { ForestPieces } from "./forestPieces";
...
export class ForestPhaserScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.pieces = new ForestPieces({
      scene: this,
      layer: this.pieceLayer,
      worldOf: (position) => this.worldOf(position),
      createTrimmedMeepleTexture: (speciesId) => this.createTrimmedMeepleTexture(speciesId),
      selectColor: SELECT,
      hiddenTint: HIDDEN_TINT
    });
  ...
  getPieceScreenPoint(pieceId: string): { x: number; y: number } | null {
    ...
    const world = this.pieces?.getPieceWorld(pieceId);
  ...
  private render(): void {
    ...
    drawForestHighlights({
      ...
      callbacks: this.cb,
      worldOf: (position) => this.worldOf(position),
      dashedRoundRect: (graphics, x, y, width, height, radius) =>
        this.dashedRoundRect(graphics, x, y, width, height, radius),
      createRoundPortraitTexture: (speciesId) => this.createRoundPortraitTexture(speciesId),
      cardSize: CARD,
      radius: RADIUS,
      selectColor: SELECT
    });
    ...
    this.pieces?.sync(vm, this.pulses, this.cb);
