...
1. **Fazer primeiro:** continuar a divisão de `apps/web/src/screens/OikosApp.tsx` com o dock de ação esquerdo (`hud-left`) e, em uma fatia posterior, o tabbar mobile. Validar cada fatia com smoke in-game real (login → Teste Local → screenshot + console). Restam remanescentes mortos no pai (valor `hudRightCollapsed` e `highlightedMovementGuideSpecies` agora sem uso) — limpar quando tocar nas fatias relacionadas.
