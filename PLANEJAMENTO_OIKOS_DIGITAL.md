...
1. **Fazer primeiro:** continuar a divisão de `apps/web/src/screens/OikosApp.tsx` (modais/diálogos inline extraídos; painel direito morto removido): seguir com os blocos de HUD que de fato renderizam — barra de espécie (`hud-species`), dock de ação esquerdo (`hud-left`) e o tabbar mobile. Validar cada fatia com smoke in-game real (login → Teste Local → screenshot + console). Restam remanescentes mortos no pai (valor `hudRightCollapsed` e `highlightedMovementGuideSpecies` agora sem uso) — limpar quando tocar nas fatias relacionadas.
