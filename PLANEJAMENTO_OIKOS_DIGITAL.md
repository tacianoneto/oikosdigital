...
## 0.1 Handoff de Refatoração — Rodada 2 (OikosApp)

Aberto em 2026-06-15. Seção temporária. Alvo único desta rodada: reduzir o god-component `apps/web/src/screens/OikosApp.tsx` (4057 linhas; 48 `useState`, 35 `useEffect`, 39 `useCallback`, 19 `useRef`, 26 handlers numa única função). Estratégia: extrair clusters coesos de estado+lógica para hooks dedicados (mesmo padrão de `useOikosSocket`/`useOpenRoomsPolling`), deixando `OikosApp.tsx` como casca de orquestração. Sem mudar comportamento, API, determinismo nem paridade local/online.
...
- Feedback/efeitos transitórios (`error`, `notice`, `threatReveal`, `macawScoreAnim`, `capuchinScoreAnim`, `turnBanner`, `floatingGains`, `travelEffects`, `turnRecap`, `hoveredSummaryCardIds`, `showJaguarScoreModal`, `expandedObjectiveCardId`, `pendingObjectiveCardId`) extraídos para `apps/web/src/hooks/useGameFeedback.ts` (`useGameFeedback`). Container puro: por serem dirigidos por efeitos grandes de game-sync com timers de auto-dismiss e refs (`lastThreatRef`) + valor derivado (`turnSummary`), só os `useState` migraram; todos os efeitos, timers, refs e derivações ficaram em `OikosApp`. Imports órfãos removidos (`CapuchinHabitatGroup`, `TurnRecapState`). `typecheck`/`test`/`build` ok; smoke in-game real (login → Teste Local → partida): toast de aviso renderiza, HUD/board/meeples intactos após recentralizar, sem erros de console.

### Ordem obrigatória recomendada

1. **Fazer primeiro:** extrair seleção de ação corrente (`selectedSpecies`, `selectedHandCardId`, `selectedCardRotation`, `selectedPieceId`, alvos de onça/lobo, `selectedRemovalPieceIds`, `cacaIlegalRemovalMode`, `selectedOpponentPlayerId`, `pendingPlacement`, `expansionPreview`/`expansionOrigin`, `movementPreview`, `landingMode`) para `useActionSelection` — o maior e mais entrelaçado.
2. Após a extração, reavaliar se os 26 handlers podem migrar junto do estado que consomem (dispatch local/online), reduzindo o corpo do componente.

### Fora de escopo nesta rodada

- `rooms.ts` (1291): máquina de estado coesa; só dividir em rodada própria se virar gargalo.
